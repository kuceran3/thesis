...
When solving a pattern matching problem there is first needed to specify what is a pattern matching which is done in the section \ref{multidimApprPattMatch}. Another section is devoted to introduction of the scientific database SciDB \ref{scidb} with its binary data format and some possible ways of working with its data.
...
When using Quadtree method on of the ways of creating it is to use the Adaptive Mesh Refinement algorithm described below \ref{AMR}.
